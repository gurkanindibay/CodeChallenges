...
public class MinimumSwaps2OtherSolutions {
    static int minimumSwaps2(int[] arr) {
        if (arr == null || arr.length == 0) return 0;

        boolean isArrayDisordered = true;
        int switchCount = 0;
        while (isArrayDisordered) {
            int leastValueIndex = 0;
            int leastValue = Integer.MAX_VALUE;
            isArrayDisordered = false;
            for (int i = 0; i < arr.length; i++) {
                if ((i + 1) < arr.length && arr[i] > arr[i + 1]) isArrayDisordered = true;
                if (arr[i] <= leastValue && (i - 1) >= 0 && arr[i - 1] > arr[i] && isArrayDisordered) {
                    leastValueIndex = i;
                    leastValue = arr[leastValueIndex];
                }
            }

            int highestValueBeforeLeastValueIndex = 0;
            for (int i = 0; i < leastValueIndex; i++) {
                if (arr[i] >= arr[highestValueBeforeLeastValueIndex]) highestValueBeforeLeastValueIndex = i;
            }

            if (leastValueIndex > 0) {
                int leastValueInArray = arr[leastValueIndex];
                arr[leastValueIndex] = arr[highestValueBeforeLeastValueIndex];
                arr[highestValueBeforeLeastValueIndex] = leastValueInArray;
                System.out.print("Swapped:" + String.valueOf(leastValueInArray) + "-" + String.valueOf(arr[leastValueIndex]) + "____");
    ...
    static int minimumSwaps(int[] arr) {
        ...
        List<Integer> unorderedList = Arrays.stream(arr).boxed().collect(Collectors.toList());

        while (unorderedList.size() > 0) {
            int smallestValue = Integer.MAX_VALUE;
            int smallestValueIndex = 0;
            for (int i = 0; i < unorderedList.size(); i++) {
                if (unorderedList.get(i) < smallestValue) {
                    smallestValue = unorderedList.get(i);
                    smallestValueIndex = i;
                }
            }

            //System.out.print("Swapped:" + String.valueOf(smallestValue) + "-" + String.valueOf(unorderedList.get(0)) + "____");
            //orderedList.forEach(v -> System.out.print(v + " "));
            //unorderedList.forEach(v -> System.out.print(v + " "));
            //System.out.println();

            unorderedList.set(smallestValueIndex, unorderedList.get(0));
            unorderedList.remove(0);
            orderedList.add(smallestValue);

            boolean isRemainingListOrdered = true;
            for (int i = 0; i < unorderedList.size() - 1; i++) {
                if (unorderedList.get(i) > unorderedList.get(i + 1)) {
                    isRemainingListOrdered = false;
                    break;
                }
            }

            if (isRemainingListOrdered) {
                orderedList.addAll(unorderedList);
                unorderedList.clear();
            }

            if (smallestValueIndex != 0) switchCount++;
    ...
    static int minimumSwaps4(int[] arr) {
        ...
        int[] referenceArray = Arrays.copyOf(arr, arr.length);
        Arrays.sort(referenceArray);

        int switchCount = 0;
        List<Integer> list = Arrays.stream(arr).boxed().collect(Collectors.toList());
        for (int i = 0; i < referenceArray.length; i++) {
            int smallestValueIndex = list.indexOf(referenceArray[i]);

            //     System.out.print("Swapped:" + String.valueOf(referenceArray[i]) + "-" + String.valueOf(arr[i]) + "____");
            ///        list.forEach(v-> System.out.print(v+" "));
            //          System.out.println();
//           System.out.println();
            if (smallestValueIndex != i) {
                switchCount++;
                list.set(smallestValueIndex, list.get(i));
                list.set(i, referenceArray[i]);
            }
        }
    ...
    static int minimumSwaps3(int[] arr) { //Timeout
        int switchCount = 0;

        int orderIndex = 0;

        Arrays.sort(arr);
        while (orderIndex < arr.length) {

            int smallestValue = Integer.MAX_VALUE;
            int smallestValueIndex = orderIndex;
            for (int i = orderIndex; i < arr.length; i++) {
                if (arr[i] < smallestValue) {
                    smallestValue = arr[i];
                    smallestValueIndex = i;
                }
            }
            if (smallestValueIndex != orderIndex) {
                switchCount++;
                arr[smallestValueIndex] = arr[orderIndex];
                arr[orderIndex] = smallestValue;
            }
            orderIndex++;
        }
